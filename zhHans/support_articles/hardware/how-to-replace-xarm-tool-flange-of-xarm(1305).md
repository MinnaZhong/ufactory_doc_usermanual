...
在更换末端法兰过程中会丢失并且重置手臂最后一个关节的零点位置。在拆卸任何部件之前，需要标记在零点位置时末端法兰 IO 接口与手臂对准位置（应与带有”UF”标志橡胶塞的在同一侧），以防止此位置丢失。

## 2. 按下急停按钮、关闭电源并拔下手臂电源线

## 3. 拆卸手臂最后一个关节的银色护盖

护盖上的螺栓被贴纸遮盖。可以用镊子将贴纸揭开后再拧下螺栓，或者直接用内六角扳手戳破贴纸拧下螺栓并拆下护盖
...
拧下固定法兰的螺栓，注意两侧中间的螺栓有 3 个垫圈（2个平垫+1个弹垫），而外侧的螺栓只有1个垫圈
...
红色为+24V，蓝色为GND。剥去约 1cm 的线皮以便压接。最佳效果是确保金属线从蓝色压线端子中露出，靠近螺栓的圆孔。将它们固定到电路板上。
